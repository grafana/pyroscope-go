...
func (s *stack) mergeDimensions() {
	...
	sort.Slice(s.dims, func(i, j int) bool {
		return s.dims[i].key < s.dims[j].key
	})
	j := 0
	for i := 1; i < len(s.dims); i++ {
		if s.dims[j].key == s.dims[i].key {
			s.dims[j].value += s.dims[i].value
		} else {
			j++
			s.dims[j] = s.dims[i]
		}
...
func (p *wallClockProfile) exportPprof(hz int64, startTime, endTime time.Time) *profile.Profile {
	...
	prof.Period = 1e9 / hz // Number of nanoseconds between samples.
